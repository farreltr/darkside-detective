	...
	public class SequencerCommandACCam : SequencerCommand {
		...
		public void Start() {
			...
			if (bridge == null) {
				...
				//---Was: if (DialogueDebug.LogWarnings) Debug.LogWarning(string.Format("{0}: Sequencer: ACCam({1}): Invalid mode '{1}'", DialogueDebug.Prefix, mode));
		...
		private void SetGameCamera(string cameraName, float smoothTime) {
			foreach (var cam in FindObjectsOfType<_Camera>()) {
				if (string.Equals(cam.name, cameraName)) {
					...
					if (smoothTime > 0) {
						mainCam.SmoothChange(smoothTime, MoveMethod.Smooth, AnimationCurve.EaseInOut(0, 0, 1, 1));
					} else {
